...
static const size_t NumCP936Pairs = sizeof CP936Pairs / sizeof CP936Pairs[0];
...
size_t CP936_to_Unicode(char **pp_cp936_char, uint16_t *utf_16, uint16_t char_for_fail)
{
	...
	*utf_16 = char_for_fail;
	...
	if (mbchar == CP936Pairs[min_index][0]) {*utf_16 = CP936Pairs[min_index][1]; return 2;}
	if (mbchar == CP936Pairs[max_index][0]) {*utf_16 = CP936Pairs[max_index][1]; return 2;}
	index = (min_index + max_index) >> 1;
	for (;;)
	{
		uint16_t cur_code = CP936Pairs[index][0];
		if (cur_code == mbchar)
		{
			*utf_16 = CP936Pairs[index][1];
			return 2;
		}
		if (cur_code < mbchar)
			min_index = index;
